...
class SVCFly:
    """ComfyUI node for Stable Virtual Camera Flythrough generation."""
    ...
    def generate_fly(self, images, seed, cfg, camera_scale, num_frames, chunk_strategy="nearest"):
        ...
        options["encoding_t"] = 1
        options["decoding_t"] = 1
        options["abort_event"] = self.abort_event
        
        # Set task type for rendering 
        task = "img2trajvid"
        ...
        # Calculate chunks for second pass
        anchor_argsort = np.argsort(input_indices + anchor_indices).tolist()
        anchor_indices_sorted = np.array(input_indices + anchor_indices)[anchor_argsort].tolist()
        gt_input_inds = [anchor_argsort.index(i) for i in range(input_c2ws.shape[0])]
        ...
        # Print progress info
        print(f"Rendering SEVA video with {num_frames} frames. First pass: {num_chunks_0} chunks, Second pass: {num_chunks_1} chunks")
        ...
        video_path = None
        for path in video_path_generator:
            if path is not None and path.endswith(".mp4"):
                video_path = path
        ...
        for frame_path in frame_files:
            frame = torch.from_numpy(iio.imread(frame_path) / 255.0).float()
            # Convert HWC to BCHW
            frame = rearrange(frame, "h w c -> 1 c h w")
            frames.append(frame)
        
        output_frames = torch.cat(frames, dim=0)
